/*
 * ════════════════════════════════════════════════════════════════
 *  ESP32 — Multi-Device Control  |  IoT Platform
 ...
 *
 *  BOARD   : ESP32 Dev Module (any ESP32 WROOM/WROVER devkit)
 *  LIBRARIES: WiFi, HTTPClient, WiFiClientSecure, ArduinoJson v6.x
 *  REQUIRES: ESP32 Arduino core >= 3.0 (analogWrite() PWM support)
...
// ── Built-in LED (GPIO2, active HIGH on most ESP32 devkits) ──────
#define PIN_LED   2
...
#define PIN_FAN 4  // GPIO4 — must be PWM-capable
...
void setup() {
  ...
  Serial.println(F("\n══════════════════════════════════"));
  Serial.println(F("  ESP32 Multi-Device Control"));
  Serial.printf( "  Devices registered : %d\n", DEVICE_COUNT);
  Serial.printf( "  Poll interval      : %lu ms\n", POLL_INTERVAL);
  Serial.println(F("══════════════════════════════════\n"));

  // Init all control pins
  pinMode(PIN_LED,   OUTPUT); digitalWrite(PIN_LED,   LOW);  // LED off
